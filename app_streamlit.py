...
st.markdown("""
Production-ready inference dashboard demonstrating the **2-Layer LSTM (T=24)** architecture 
proposed for KSE 2026. Mapping historical meteorological telemetry to next-hour particulate concentration with **R² = 0.9523**.
""")
st.divider()
...
selected_case = st.sidebar.selectbox("Select Scenario Preset", list(REPRESENTATIVE_CASES.keys()))

case_cfg = REPRESENTATIVE_CASES[selected_case]

if case_cfg is not None:
    st.sidebar.info(f"**Context:** {case_cfg['desc']}")
...
# --- EXECUTE FORECAST AUTOMATICALLY ON LOAD/CHANGE ---
if lstm_model is None or scaler is None:
    ...
    # --- RENDER RESULTS DIRECTLY ON MAIN SCREEN ---
